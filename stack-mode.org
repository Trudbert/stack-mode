...
#+begin_src emacs-lisp :noweb strip-export :tangle elisp/stack-mode.el
...
#+begin_src emacs-lisp :noweb yes :tangle "elisp/stack-network-mode.el"
...
#+end_src
  ...
  Since it is an integrated part of =stack-mode= and
dispatches to several sibling modes, it is imperative that
=stack-mode= (and all of its child modes) be available upon entering
=stack-network-mode=.
...
As you can see, these could easily be calculated from the existing
data.
