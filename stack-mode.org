...
#+BEGIN_SRC elisp
  (defvar stack-network-favorites-file
    ...
    "The filepath to a whitespace-delimited file of
  stack_api_tokens corresponding to the user's favorite sites.
  This variable is used by `STACK-NETWORK-MODE' to group the user's
  favorite sites to the top for easy access.")
#+END_SRC
...
#+BEGIN_SRC elisp
  ...
  `STACK-NETWORK-FAVORITES-FILE'"
    (if (not (file-exists-p stack-network-favorites-file))
        (kill-buffer
         (save-buffer
          (find-file-noselect stack-network-favorites-file))))
    (with-temp-buffer
